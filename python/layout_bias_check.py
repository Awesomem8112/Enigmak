"""
ENIGMAK rc.6 layout and safety bias checker.
...
def main():
    failures = []
    print("=" * 72)
    print("ENIGMAK rc.6 Layout Bias / Safety Checker")
    print("=" * 72)
    ...
    for name in LAYOUT_NAMES:
        ...
        check(covers_input and covers_output and roundtrip,
              f"{name} is a full permutation", failures)
    ...
    for name in LAYOUT_NAMES:
        ...
        check(counts == {97},
              f"{name} output frequency is exactly uniform", failures)
    ...
    for i, left in enumerate(LAYOUT_NAMES):
        for right in LAYOUT_NAMES[i + 1:]:
            matches = sum(
                1 for c in ALPHA if km['layout_maps'][left][c] == km['layout_maps'][right][c])
            pair_matches.append(matches)
    average_matches = sum(pair_matches) / len(pair_matches)
    max_matches = max(pair_matches)
    print(
        f"  Average shared mappings per layout pair: {average_matches:.2f} (random expectation about 1)")
    print(f"  Maximum shared mappings in one pair: {max_matches}")
    check(average_matches < 3.0 and max_matches < 10,
          "inter-layout overlap stays near random", failures)
    ...
    for _ in range(32):
        ...
        bit_values.append(
            round(calc_key_strength(sample_parsed)['family_bits'], 1))
    unique_outputs = len(set(samples))
    unique_profiles = len(set(profiles))
    unique_bits = len(set(bit_values))
    print(
        f"  Colemak E mapped to {unique_outputs} unique outputs across 32 keys")
    print(
        f"  Generated {unique_profiles} unique key profiles and {unique_bits} unique strength values")
    check(unique_outputs >= 12, "cross-key mapping varies", failures)
    check(unique_profiles >= 6,
          "keygen is not locked to one theoretical profile", failures)
    check(unique_bits >= 6 and 151.5 not in set(bit_values),
          "keygen is not producing fixed 151.5-bit keys", failures)

    print("\n[5] Generated keys meet the rc.6 minimum accepted family strength")
    for index in range(1000):
        sample_key = generate_key()
        sample_parsed = parse_key(sample_key)
        bits = calc_key_strength(sample_parsed)['family_bits']
        check(bits >= MIN_GENERATED_KEY_BITS,
              f"key {index + 1:02d} has {bits:.1f} bits", failures)

    print("\n[6] Corruption buffer is fixed length")
    check(MAX_CORRUPT_LEN == 4096, "MAX_CORRUPT_LEN is exactly 4096", failures)
    check(len(_corrupt_buffer()) == 4096,
          "_corrupt_buffer() emits exactly 4096 code points", failures)

    print("\n[7] Tampered rc.6-stream ciphertext fails closed")
    ...
    for label, damaged in (
        ...
        check(
            not result.get('success') and not result.get(
                'verified') and result.get('plaintext') == '',
